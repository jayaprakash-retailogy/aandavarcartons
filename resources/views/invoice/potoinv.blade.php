...
<body data-spy="scroll" data-target="#navSection" data-offset="100">
                            ...
                            <div class="widget-content widget-content-area">                             
                                ...
                                @csrf
                                    <div class="form-row">
                                        <div class="col-md-3 mb-4">
                                            ...
                                            <select class="placeholder js-states form-control select" name="purchaseorderid" id="purchaseorderid" required>
                                                ...
                                                <option value="{{ $purchaseorder->id }}" {{ (isset($podata) && !empty($podata) && $purchaseorder->id == $podata->id?'selected':'')}}>{{ $purchaseorder->purchaseordernumber }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3 mb-4">
                                    ...
                                    <div class="table-responsive mt-4">
											<table class="table table-stripped table-center" id="purchaseorderitemstable">
												<thead>
													<tr>
                                                        <th width="2%"><input id="check_all" class="formcontrol" type="checkbox"/></th>
                                                        <!-- <th>s.no</th> -->
                                                        <th>description(_for_)PurchaseOrders</th>
                                                        <th>hsn/sac</th>
                                                        <th>quantity</th>
														<th>unit</th>
														<th>rate</th>
                                                ...
                                                @if(isset($purchaseorderitems) && !empty($purchaseorderitems))
                                                    ...
                                                    @if($purchaseorderitem->quantity-$purchaseorderitem->deliveredqty != '0')
													<tr>
                                                        ...
                                                        <!-- <td>
                                                            ...
                                                            <input type="hidden" name="poitemid[]" id="poitemid_{{ $loop->iteration }}" class="form-control" autocomplete="off" required value="{{ $purchaseorderitem->id }}">
                                                        
                                                        <td>
                                                            <input type="text" name="description[]" id="description_{{ $loop->iteration }}" class="form-control" autocomplete="off" required value="{{ $purchaseorderitem->description }} {{ $purchaseorderitem->length }}x{{ $purchaseorderitem->breadth }}x{{ $purchaseorderitem->height }}">
                                                        </td>
														<td>
                                                            <input type="text" name="hsnsac[]" id="hsnsac_{{ $loop->iteration }}" class="form-control" autocomplete="off" required value="{{ $purchaseorderitem->hsnsac ?? '48191010' }}">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="quantity[]" id="quantity_{{ $loop->iteration }}" class="form-control quantity changesNo" autocomplete="off" value="{{ $purchaseorderitem->quantity }}" required>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="unit[]" id="unit_{{ $loop->iteration }}" class="form-control" required autocomplete="off" value="{{ $purchaseorderitem->unit ?? 'pcs' }}" required>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="rate[]" id="rate_{{ $loop->iteration }}" class="form-control rate changesNo" required autocomplete="off" value="{{ $purchaseorderitem->costperbox ?? '' }}" required>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="discount[]" id="discount_{{ $loop->iteration }}" class="form-control discount changesNo" required autocomplete="off" value="{{ $purchaseorderitem->discount ?? '0' }}">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="amount[]" id="amount_{{ $loop->iteration }}" class="form-control amount" required autocomplete="off" value="{{ $purchaseorderitem->amount ?? '' }}" readonly>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="taxable[]" id="taxable_{{ $loop->iteration }}" class="form-control taxable" required autocomplete="off" value="{{ $purchaseorderitem->taxable ?? '' }}" readonly>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="cgst[]" id="cgst_{{ $loop->iteration }}" class="form-control cgst changesNo" placeholder="%" value="{{ $purchaseorderitem->cgst ?? ''}}" required>

                                                            <input type="hidden" name="cgstvalue[]" id="cgstvalue_{{ $loop->iteration }}" class="form-control cgstvalue" value="">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="sgst[]" id="sgst_{{ $loop->iteration }}" class="form-control sgst changesNo" placeholder="%" value="{{ $purchaseorderitem->sgst ?? ''}}" required>

                                                            <input type="hidden" name="sgstvalue[]" id="sgstvalue_{{ $loop->iteration }}" class="form-control sgstvalue" value="">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="igst[]" id="igst_{{ $loop->iteration }}" class="form-control igst changesNo" placeholder="%" value="{{ $purchaseorderitem->igst ?? ''}}" required>

                                                            <input type="hidden" name="igstvalue[]" id="igstvalue_{{ $loop->iteration }}" class="form-control igstvalue" value="">
                                                        </td>
													</tr>
                                                    @endif
                                                    @endforeach
                                                    ...
                                                    <tr>    
                                                        ...
                                                        <td>
                                                            <input type="text" name="cgst[]" id="cgst_1" class="form-control cgst changesNo" placeholder="%" value="" required>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="sgst[]" id="sgst_1" class="form-control sgst changesNo" placeholder="%" value="" required>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="igst[]" id="igst_1" class="form-control igst changesNo" placeholder="%" value="" required>
                                                        </td>
													</tr>
                                                    @endif
												</tbody>
    ...
    <script>
        ...
        */ 
        ...
        var i=$('table#purchaseorderitemstable tr').length;
        ...
        $("#acaddmore").on('click',function(){
           ...
           // sno = j + 1;
            ...
            $('table#purchaseorderitemstable').append(html);
            calculateTotal();
            i++;
        });
